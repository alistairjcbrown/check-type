    ...
    suite("Check Type", function() {
        ...
        suite("Has function", function() {
            ...
            test("should return true when path exists", function() {
                var has_attribute_path = check(fixtures.has_attribute).has("hello.world.foo bar.baz"),
                    has_property_path = check(fixtures.has_property).has("hello.world.foo bar.baz");
                expect(has_attribute_path).to.be.true;
                expect(has_property_path).to.be.true;
            });

            test("should return true when path exists with escaped period", function() {
                var has_attribute_with_period_path = check(fixtures.has_attribute_with_period).has("hello.world.foo\\.bar.baz"),
                    has_property_with_period_path = check(fixtures.has_property_with_period).has("hello.world.foo\\.bar.baz");
                expect(has_attribute_with_period_path).to.be.true;
                expect(has_property_with_period_path).to.be.true;
            });

            test("should return false when path does not exist", function() {
                var has_attribute_path = check(fixtures.has_attribute).has("hello.everyone.foo bar.baz"),
                    has_property_path = check(fixtures.has_property).has("hello.everyone.foo bar.baz");
