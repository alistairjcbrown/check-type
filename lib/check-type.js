        ...
        check = function(value) {
            ...
            var response = {},
                get_levels_from_path = function(path) {
                    var path_pieces, path_levels;

                    // Be strict about splitting the string;
                    // Check that the period has not been escaped.
                    path_pieces = path.split(/(^|[^\\])(\\\\)*\./g);

                    // Rebuild split levels using captured groups
                    path_levels = _.reduce(path_pieces, function(memo, level_name, index) {
                        level_name = _.isUndefined(level_name) ? "" : level_name;
                        if (index % 3 === 0) {
                            memo.push(level_name);
                        } else {
                            memo[Math.floor(index / 3)] += level_name;
                        }
                        return memo;
                    }, []);

                    // Replace escape characters for matching
                    return _.map(path_levels, function(path_level) {
                        return path_level.replace(/\\\./g, ".")
                                         .replace(/\\\\/g, "\\");
                    });
                };
